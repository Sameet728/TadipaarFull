...
const { haversineKm, isPointInPolygon } = require('../utils/geoUtils');
const { verifyFace }  = require('../utils/faceVerification');
const { getDivisionByName } = require('../utils/kmlParser');
...
const checkIn = async (req, res, next) => {
  try {
    ...
    const areasResult = await query(
      'SELECT area_name, latitude, longitude, radius_km, is_confinement, area_type FROM restricted_areas WHERE criminal_id = $1',
      [criminalId]
    );
    ...
    for (const area of areasResult.rows) {
      ...
      if (area.area_type === 'polygon') {
        const div = getDivisionByName(area.area_name);
        if (div) {
          isInside = isPointInPolygon(lat, lng, div.polygon);
        } else {
          console.warn(`Division ${area.area_name} not found`);
        }
      } else {
        dist = haversineKm(lat, lng, parseFloat(area.latitude), parseFloat(area.longitude));
        isInside = dist <= parseFloat(area.radius_km);
      }

      if (isConfinement) {
        // Confinement zone: externee MUST stay INSIDE
        if (!isInside) {
          status          = 'non_compliant';
          if (area.area_type === 'polygon') {
            violationReason = `Outside confinement division: "${area.area_name}"`;
          } else {
            violationReason = `Outside confinement zone: "${area.area_name}" (${dist.toFixed(2)} km from center, must stay within ${area.radius_km} km)`;
          }
          break;
        }
      } else {
        // Restriction zone: externee MUST stay OUTSIDE
        if (isInside) {
          status          = 'non_compliant';
          if (area.area_type === 'polygon') {
            violationReason = `Found inside restricted division: "${area.area_name}"`;
          } else {
...
const getMyAreas = async (req, res, next) => {
  try {
    ...
    const areasResult = await query(
      `SELECT id, area_name, latitude, longitude, radius_km, is_confinement, area_type, created_at
    ...
    return res.status(200).json({
      ...
      areas: areasResult.rows.map((r) => ({
        ...
        isConfinement: r.is_confinement === true,
