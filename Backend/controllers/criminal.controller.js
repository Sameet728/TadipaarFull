  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO criminals
         (name, login_id, password, plain_password, phone, email, address, case_number,
      ...
      [
        name.trim(), loginId.trim(), hashed, password,
    ...
    // If externFromPune is true, create Pune city confinement zone
    if (externFromPune === 'true' || externFromPune === true) {
      await client.query(
    ...
    if (divisions) {
      let parsedDivisions = [];
      try {
        parsedDivisions = typeof divisions === 'string' ? JSON.parse(divisions) : divisions;
      } catch (e) { console.error('Failed to parse divisions', e); }
      
      for (const divName of parsedDivisions) {
        const div = getDivisionByName(divName);
        if (div) {
          await client.query(
            `INSERT INTO restricted_areas
    ...
    return res.status(201).json({
      ...
      criminal: {
        ...
        photoUrl:          c.photo_url,
        createdAt:         c.created_at,
        externFromPune:    externFromPune === 'true' || externFromPune === true,
      },
    });
  } catch (err) {
