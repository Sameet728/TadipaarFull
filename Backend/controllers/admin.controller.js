...
const { getDivisions, getDivisionByName } = require('../utils/kmlParser');
  ...
  try {
    ...
    const result = await query(
      `SELECT
         ...
         c.externment_section, c.period_from, c.period_till,
         c.residence_address, c.photo_url, c.plain_password,
         ps.id   AS police_station_id,
         ps.name AS police_station,
         aa.id   AS acp_area_id,
         aa.name AS acp_area,
    ...
    const areas = await query(
      `SELECT id, area_name, latitude, longitude, radius_km, created_at,
              source_zone_id, source_police_station_id, area_type, is_confinement
    ...
    return res.status(200).json({
      ...
      criminal: {
        ...
        zone:              c.zone,
        createdAt:         c.created_at,
        ...(req.user?.role === 'CP' && { passwordVisible: c.plain_password })
      },
...
// ─────────────────────────────────────────────────────────
const getDivisionsList = async (req, res, next) => {
  try {
    const divisions = getDivisions();
    // Cache the heavy PCMC polygon for 24 hours on the client
...
const addRestrictedArea = async (req, res, next) => {
  try {
    ...
    const { areaName, latitude, longitude, radiusKm, divisionName } = req.body;

    if (divisionName) {
      const div = getDivisionByName(divisionName);
      if (!div) {
        return res.status(404).json({ success: false, message: 'Division not found.' });
      }
      try {
        const result = await query(
          `INSERT INTO restricted_areas (criminal_id, area_name, latitude, longitude, radius_km, area_type, is_confinement)
           VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
          [criminalId, div.name, 0, 0, 0, 'polygon', true]
        );
        return res.status(201).json({ success: true, message: 'Area added.', area: result.rows[0] });
    ...
    if (!areaName || latitude == null || longitude == null) {
      return res.status(400).json({
        success: false,
        message: 'Either policeStationId, zoneId, divisionName, or legacy fields (areaName, latitude, longitude) are required.',
      });
    }
    const result = await query(
