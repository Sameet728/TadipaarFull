...
// ── These MUST be outside the component so React doesn't

//    recreate them on every keystroke (causes keyboard close) ──
...
const INP = 'w-full bg-slate-50 border border-slate-300 rounded px-4 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:border-[#1E3A8A] focus:ring-1 focus:ring-[#1E3A8A] transition-colors'
...
    adminAPI.get('/criminal/meta/zones-stations')
...
  const filteredPS = acpAreaId

    ? meta.policeStations?.filter(p => String(p.acp_area_id) === String(acpAreaId))

    : meta.policeStations || []


  const handleZone = useCallback((v) => { setZoneId(v); setAcpAreaId(''); setPoliceStationId('') }, [])

  const handleAcp  = useCallback((v) => { setAcpAreaId(v); setPoliceStationId('') }, [])
...
      <p className="text-xl font-black text-[#1E3A8A] tracking-widest uppercase">Registration Successful</p>

      <p className="text-sm font-bold text-slate-400 tracking-widest uppercase">Redirecting...</p>
...
            <p className="text-[10px] font-bold text-slate-400 mt-2 tracking-widest">JPEG / PNG — MAX 10 MB</p>
